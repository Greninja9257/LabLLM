...
final class TutorialState: ObservableObject {
    ...
    let steps: [Step] = [
        ...
        .init(phase: "Simple", title: "Install pre-training data", message: "Search or choose a recommended corpus, inspect the dataset card, then install it. Installed data is written to disk and stays available next session.", section: .dataset, action: .corpusAdded, kind: .task, checkpoint: "Corpus installed"),
        .init(phase: "Simple", title: "Start pretraining", message: "Pick the corpora for this run in the training data panel, then start a small run. Simple mode builds the tokenizer when training begins.", section: .training, action: .trainingStarted, kind: .task, checkpoint: "Training started"),
        ...
        .init(phase: "Simple", title: "Install fine-tuning data", message: "Browse instruction or conversation datasets, inspect the rendered README, then install compatible rows. You choose how much of each to use in Training.", section: .fineTuneData, action: .fineTuneSourceAdded, kind: .task, checkpoint: "Fine-tuning rows installed"),
        ...
        .init(phase: "Advanced", title: "Manage checkpoints", message: "Open Checkpoints to load, continue, or organize the saved runs that belong to the model selected in the top-left box.", section: .checkpoints, action: .modelsOpened, kind: .visit, checkpoint: "Checkpoints opened"),
